...
# ### Active Learning
@of("active learning")
def activeLearning(self:DATA, score=lambda B,R,t: B-R, generate=None, faster=True ):
  ...
  def guess(todo:rows, done:rows) -> rows:
    ...
    if generate:
      ...
      key  = lambda r: score(best.loglike(r, len(done), 2), rest.loglike(r, len(done), 2), the.iter)
